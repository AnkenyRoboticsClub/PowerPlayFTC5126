...
@TeleOp
public class TeleV7_OneGamepad extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            left.setPower(gamepad1.right_trigger * 0.5 - 0.25);
            right.setPower(gamepad1.right_trigger * -0.5 + 0.25);
            //lastEncoderPosition = encoderPosition;
            encoderReal = (int)Math.round(MotorSlide.getCurrentPosition());
            
            if (Math.abs(gamepad1.right_stick_y) > 0.1){
                encoderPosition = encoderReal + (int)Math.round(gamepad1.right_stick_y) * 400;
            }
            
            if (gamepad1.right_stick_y < -0.1) {
                MotorSlide.setPower(1); //up
            } if (gamepad1.right_stick_y > 0.1){
                MotorSlide.setPower(0); //down
            } else {
                MotorSlide.setPower(1); //auto
            }
